...
class OutputProxyBase(ABC):
    ...
    @abstractmethod
    def filter_data(self, in_data):
        ...

    @abstractmethod
    def check_data(self, in_data):
        ...


class SpecialOutput(OutputProxyBase):

    def __init__(self, original_output, output_file, data_filter: list[callable] = None, checks: list[callable] = None):
        self.original_output = original_output
        self.output_file = pathmaker(output_file)
        self.filter = data_filter if data_filter is not None else []
        self.checks = checks if checks is not None else []
        if os.path.isdir(os.path.dirname(self.output_file)) is False:
            os.makedirs(os.path.dirname(self.output_file))
        if os.path.isfile(self.output_file):
            os.remove(self.output_file)

    def write(self, in_data):
        data = self.filter_data(in_data)
        if self.check_data(data) is True:
            print(data, file=self.original_output)
            with open(self.output_file, 'a') as f:
                f.write(data + '\n')

    def filter_data(self, in_data):
        data = in_data
        if self.filter in [None, []]:
            return data
        for data_filter in self.filter:
            data = data_filter(data)
        return data

    def check_data(self, in_data):
        if self.checks in [None, []]:
            return True
        for check in self.checks:
            if check(in_data) is False:
                return False
        return True
