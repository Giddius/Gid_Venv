...
class RequirementItem(namedtuple("RequirementItem", ['name', 'version_operator', 'version', 'install_instructions', 'extra_specifier', 'enabled'])):
    ...
    def install(self, activation_script, stdout, stderr, extra_install_instructions: list = None, verbose=False):
        if self.enabled:
            ...
            if verbose:
                stdout(Back.YELLOW + f"#### Installing {self.name.upper()} ####\n")
            ...
            if cmd.returncode != 0 or cmd.stderr != '':
                stderr(Back.RED + f"--- ERROR with installing pypi package {self.name.upper()} ---")
            if verbose:
                stdout(cmd.stdout)
            stderr(cmd.stderr)
            ...
            if cmd.returncode == 0:
                stdout(f"{'-'*100}")
                stdout("- ################ " + Fore.WHITE + f"SUCCESSFULLY installed {Back.GREEN}{self.name.upper()} ")
...
class PersonalRequiredItem(namedtuple("PersonalRequiredItem", ['name', 'path', 'enabled'])):
    ...
    def install(self, activation_script, stdout, stderr, extra_install_instructions: list = None, verbose=False):
        if self.enabled:
            ...
            if verbose:
                stdout(Back.YELLOW + f"#### Installing flit dev {self.name.upper()} from {path} ####\n")
            command = [activation_script, '&&', 'pushd', path, '&&', "flit", 'install', '-s', '&', "popd"]
            cmd = subprocess.run(command, capture_output=True, check=False, shell=True, text=True)
            if cmd.returncode != 0:
                stderr(Back.RED + f"--- ERROR with installing flit dev package {self.name.upper()} ---")
            cmd_out = cmd.stdout
...
class GithubRequiredItem(namedtuple("GithubRequiredItem", ['name', 'url', 'enabled'])):
    ...
    def install(self, activation_script, stdout, stderr, extra_install_instructions: list = None, verbose=False):
        if self.enabled:
            if verbose:
                stdout(Back.YELLOW + f"#### Installing from Github {self.name.upper()} ####\n")
            url = self.url
            ...
            cmd = subprocess.run(command, capture_output=True, check=False, shell=True, text=True)
            if cmd.returncode != 0:
                stderr(Back.RED + f"--- ERROR with installing Github package {self.name.upper()} ---")
            cmd_out = cmd.stdout
